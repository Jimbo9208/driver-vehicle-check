...
LOGO_URL = os.getenv(
    "LOGO_URL",
    "https://drive.google.com/uc?export=view&id=1djWHjQFdDGO7qvrd4iMYTN8wpCiV_FD0")

# Google Drive
GDRIVE_ROOT = os.getenv("GDRIVE_ROOT", "Vehicle Checks")
GOOGLE_SERVICE_ACCOUNT_JSON = os.getenv(
    "GOOGLE_SERVICE_ACCOUNT_JSON",
    "")  # JSON blob in env (optional)
GOOGLE_APPLICATION_CREDENTIALS = os.getenv(
    "GOOGLE_APPLICATION_CREDENTIALS",
    "")  # path to JSON file (e.g., /etc/secrets/xxx.json)
...
def _load_drive_service():
    ...
    if GOOGLE_SERVICE_ACCOUNT_JSON:
        ...
        try:
            ...
            # Fallback in case someone pasted Python-like dict text
            # nosec - controlled admin env
            info = json.loads(json.dumps(eval(GOOGLE_SERVICE_ACCOUNT_JSON)))
...
def send_email(subject: str, html_body: str):
    ...
    msg["To"] = [addr.strip() for addr in MAIL_TO.split(",") if addr.strip()]
    msg.set_content(
        "This email contains HTML content. Please use an HTML-compatible client.")
    msg.add_alternative(html_body, subtype="html")
    with smtplib.SMTP(MAIL_HOST, MAIL_PORT) as s:
        s.starttls()
        if MAIL_USER and MAIL_PASS:
...
def check():
    ...
    if request.method == "POST":
        ...
        if not (
                f_dash and f_dash.filename and f_front and f_front.filename and f_rear and f_rear.filename):
            flash("Dashboard, Front, and Rear photos are required.", "error")
        ...
        def _upload_file(file_storage, label):
            ...
            fname = _sanitize_filename(
                f"{vehicle_reg}_{label}_{driver_name}_{ts}.jpg")
            fid, link = _drive_upload(
                drive, folder_id, stream, fname, "image/jpeg")
            return fid, link
        ...
        if "photo_defects" in request.files:
            for fs in request.files.getlist("photo_defects"):
                if fs and fs.filename:
                    fid, lnk = _upload_file(fs, "Defect")
                    defect_ids.append(fid)
                    defect_links.append(lnk)
    ...
    # 'html' is your new rendered template
    pdf_bytes = pdfkit.from_string(html, False)
    pdf_stream = io.BytesIO(pdf_bytes)
    pdf_name = _sanitize_filename(f"{vehicle_reg}_CheckSummary_{ts}.pdf")
    pdf_id, pdf_link = _drive_upload(
        drive, folder_id, pdf_stream, pdf_name, "application/pdf")
...
html = render_template_string(
    ...
    try:
        send_email(
            subject=f"{APP_NAME}: {vehicle_reg} check submitted",
            html_body=html,
        )
    except Exception as e:
        # don't block the flow on email errors
    ...
    # Finished POST work -> go to success page
    return redirect(
        url_for(
            "success",
            check_id=new_id,
...
# ---- Outside POST (but still inside def check) ----
return render_template_string(
    CHECK_HTML,
    title="Vehicle Check",
    checklist=checklist_items,
